...
exports.getCompanyApplications = catchAsync(async (req, res, next) => {
  ...
  const applicationsQuery = JobApplication.find({ jobId: { $in: jobIds } })
    .sort({ appliedAt: -1 })
    .skip(skip)
    .limit(limit)
  ...
  const applications = await Promise.all(applicationsRaw.map(async (app) => {
    ...
    let skillsList = [];
    if (app.seekerId) {
        const seekerSkills = await SeekerSkill.find({ seekerId: app.seekerId._id }).populate('skillId');
        skillsList = seekerSkills.map(s => s.skillId.name);
    }

    const email = app.seekerId && app.seekerId.authId ? app.seekerId.authId.email : "N/A";
    const experience = app.seekerId ? `${app.seekerId.yearsOfExperience} years` : "0 years";

    return {
      id: app._id,
      candidateName: app.seekerId ? app.seekerId.fullName : "Unknown Candidate",
      email: email,
...
exports.updateApplicationStatus = catchAsync(async (req, res, next) => {
  ...
  if (application.jobId.companyId.toString() !== company._id.toString()) {
     return next(new AppError("You are not authorized to manage this application", 403));
  }
