...
// 1. التحقق من وجود اليوزر والـ CV
  ...
  if (!availableJobs || availableJobs.length === 0) {
    return next(new AppError('No published jobs available for matching at the moment.', 404));
  }
  ...
  const aiMatches = await aiService.matchJobs(seeker.cvUrl, userProfilePayload, jobsListPayload);

  if (!aiMatches || aiMatches.length === 0) {
    return next(new AppError('AI failed to find suitable matches for your profile.', 404));
  }
  ...
  const finalResults = aiMatches.map(aiMatch => {
    ...
    const fullJobDetails = populatedJobs.find(job => job._id.toString() === aiMatch.job_id);
  ...
  await JobRecommendation.deleteMany({ seekerId: seeker._id, recommendationSource: 'cv_matching' });
